...
import { TweenLite, Power1, Expo, Quad } from "gsap";
import { SharedService } from "src/app/shared/shared-service";
import { Subscription } from "rxjs";

@Component({
  selector: "app-grid-to-full-screen",
  templateUrl: "./grid-to-full-screen.component.html",
  styleUrls: ["./grid-to-full-screen.component.scss"]
})
export class GridToFullScreenComponent implements OnInit {
  @ViewChild("shaderEffectItem") shaderEffectItem: ElementRef;
  @ViewChild("shaderEffectItemFull") shaderEffectItemFull: ElementRef;
  @ViewChild("shaderEffectItemContainer") shaderEffectItemContainer: ElementRef;
  @ViewChild("shaderEffectItemsWrapper") shaderEffectItemsWrapper: ElementRef;
  ...
  transformations = {
    ...
    simplex: (props) => {
      let seed = this.ensureFloat(props.seed || 0);
      let amplitudeX = this.ensureFloat(props.amplitudeX || 0.5);
      let amplitudeY = this.ensureFloat(props.amplitudeY || 0.5);
      let frequencyX = this.ensureFloat(props.frequencyX || 1);
      let frequencyY = this.ensureFloat(props.frequencyY || 0.75);
      let progressLimit = this.ensureFloat(props.progressLimit || 0.5);
      return `
    ...
    wavy: (props) => {
      const seed = this.ensureFloat(props.seed || 0);
      const amplitude = this.ensureFloat(props.amplitude || 0.5);
      const frequency = this.ensureFloat(props.frequency || 4);
      return `
        float limit = 0.5;
...
`;
  ...
  ngAfterViewInit(): void {
    this.transitionEffectDuration = 1.2;
    this.options = {
      scrollContainer: window,
      activation: { type: "mouse" },
      timing: {
        duration: this.transitionEffectDuration,
        type: "sameEnd",
        sections: 1,
        latestStart: 0.5
      },
      ...
      onToFullscreenStart: ({ index }) => {
        //currentIndex = index;
        this.shaderEffectItem.nativeElement.style.opacity = 0;
        this.uniforms.uSeed.value = index * 10;
        this.toggleFullview();
      },
      onToGridFinish: ({ index, lastIndex }) => {
        this.shaderEffectItem.nativeElement.style.opacity = 1;
        this.shaderEffectItemFull.nativeElement.classList.remove(
    ...
    this.container = this.shaderEffectItemContainer.nativeElement;
    this.itemsWrapper = this.shaderEffectItemsWrapper.nativeElement;
    this.backToGridControl = this.backToGrid.nativeElement;
    ...
    this.uniforms = {
      ...
      uProgressByParts: new THREE.Uniform(
        this.options.timing.type === "sections"
      ),
    ...
    let imageSmall = {
      element: this.shaderEffectItem,
      image: this.shaderEffectItem.nativeElement
    };

    let imageFull = {
      element: this.shaderEffectItemFull,
      image: this.shaderEffectItemFull.nativeElement
    };

    var imageSet = { small: undefined, large: undefined };
    imageSet.small = imageSmall;
    imageSet.large = imageFull;
  ...
  toggleFullview = () => {
    if (this.isFullscreen) {
      // TweenLite.to(fullviewItem.querySelector(".fullview__item-title"), 0.2, {
      //   ease: Quad.easeOut,
      //   opacity: 0,
      //   x: "5%"
      // });

      // gsap.to(this.backToGridControl, {
      //   duration: 0.2,
      //   scale: 0,
      //   opacity: 0,
      //   ease: Expo.easeOut
      ...
      // TweenLite.to(this.backToGridControl, 0.2, {
      //   ease: Quad.easeOut,
      //   opacity: 0,
      //   scale: 0
      // });

      this.backToGridControl.style.opacity = 0;
      this.backToGridControl.style.scale = 0;

      this.toGrid();
    } else {
      this.shaderEffectItemFull.nativeElement.classList.add(
        "fullview__item--current"
      );

      // TweenLite.to(fullviewItem.querySelector(".fullview__item-title"), 1, {
      //   ease: Expo.easeOut,
      //   startAt: { x: "5%" },
      //   opacity: 1,
      //   x: "0%",
      ...
      //   scale: 1,
      //   opacity: 1,
      //   delay: this.transitionEffectDuration * 0.6,
      //   startAt: { scale: 0 },
      //   ease: Expo.easeOut
      // });

      // TweenLite.to(this.backToGridControl, 1, {
      //   ease: Expo.easeOut,
      //   startAt: { scale: 0 },
      //   opacity: 1,
      //   scale: 1,
      ...
      this.backToGridControl.style.opacity = 1;
      this.backToGridControl.style.scale = 1;
    }
  };
  ...
  recalculateUniforms(ev) {
    ...
    if (xIndex === false && yIndex === false) {
      closestCorner = 0;
    } else if (xIndex === false && yIndex === true) {
      closestCorner = 1;
    } else if (xIndex === true && yIndex === false) {
      closestCorner = 2;
    } else if (xIndex === true && yIndex === true) {
      closestCorner = 3;
    }
  ...
  init() {
    ...
    const transformation = isFunction(this.options.transformation.type)
      ...
      : this.transformations[this.options.transformation.type](
          this.options.transformation.props
        );
    const shaders = this.generateShaders(
      this.activations[this.options.activation.type],
      transformation
    );
  ...
  generateShaders(activation, transform) {
    var vertex = `
      ${this.vertexUniforms}
      ${this.cubicBeizer}
      ${this.simplex}
